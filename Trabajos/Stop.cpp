...
void imprimirResultados(const vector<ResultadoRonda> &resultados, const string &nombreJugador)
{
    ...
    ofstream archivo("resultados.bin", ios::binary | ios::app);
    ...
    archivo << left << setw(10) << "Letra" << setw(20) << "Tiempo (s)" << setw(10) << "Puntos" << endl;
    double tiempoTotal = 0;
    int puntosTotales = 0;

    for (const auto &r : resultados)
    {
        archivo << setw(10) << r.letra << setw(20) << r.tiempo << setw(10) << r.puntos << endl;
        tiempoTotal += r.tiempo;
        puntosTotales += r.puntos;
    }

    archivo << setw(10) << "Total" << setw(20) << tiempoTotal << setw(10) << puntosTotales << endl;

    // Cerrar el archivo
    archivo.close();
}

void copiarBinarioATexto() {
    ifstream archivoBinario("palabras.bin", ios::binary);
    if (!archivoBinario.is_open()) {
        cout << "El archivo palabras.bin no existe. No se puede copiar al archivo de texto." << endl;
        return;
    }

    ofstream archivoTexto("palabras.txt", ios::app); // Abrir en modo de añadir
    if (!archivoTexto.is_open()) {
        cout << "No se pudo abrir el archivo palabras.txt" << endl;
    ...
    // Leer y escribir cada registro del archivo binario al archivo de texto
    while (archivoBinario.read(reinterpret_cast<char*>(&resultado), sizeof(ResultadoRonda))) {
        archivoTexto << resultado.letra << " " << resultado.tiempo << " " << resultado.puntos << endl;
    }
